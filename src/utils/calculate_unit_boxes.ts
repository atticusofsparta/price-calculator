 ...
 */
export class ConnectedGatewayOracle implements ArweaveOracle {

	gatewayUrl:string;
	constructor () {
		
			const urlFragments =window.location.host.split(".")
			const url = [urlFragments[1], urlFragments[2]]
			this.gatewayUrl = url.join(".")
	...
	async getWinstonPriceForByteCount (byteCount:ByteCount): Promise<Winston> {
		...
		await axios.get(`https://www.${this.gatewayUrl}/price/${byteCount}`).then((res)=>{
			price = new BigNumber(+res.data)
		})

			
			console.log(price.valueOf())
			if (price.valueOf() === "NaN") {
				throw new Error("Unable to get price")
			}
			return new Winston(new BigNumber(price))
		} catch (error) {
			console.error(error)
			return new Winston(new BigNumber(1))
